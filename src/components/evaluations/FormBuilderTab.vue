...
const responseTypeOptions = [
  { value: 'rating', label: 'Stars Rating (1-5)' },
  { value: 'text', label: 'Text Comment Response' }
]
...
async function addQuestion() {
  ...
  const result = await evaluationsStore.addTemplateQuestion(workingTemplate.value.id, {
    question: 'Specify standard evaluation criteria targeting key performance parameters…',
...
async function openArchiveModal() {
  ...
  try {
    ...
    if (!list.length) {
      archiveEmptyHint.value =
        'No archived templates were returned. If the backend stores archives differently, check the console diagnostics from fetchArchivedTemplates.'
    }
  } catch (err) {
    archiveEmptyHint.value = err.message || 'Failed to load archived templates.'
  } finally {
...
async function confirmArchiveImport() {
  ...
  await evaluationsStore.duplicateTemplate(selectedArchiveTemplateId.value, {
    name: importName.value || 'Imported Template',
    archive_source: true
  })
  await evaluationsStore.fetchTemplates()
  if (evaluationsStore.templates.length) {
  ...
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
      <div class="flex items-center justify-between flex-wrap gap-4">
        <div class="space-y-1">
          <h4 class="text-md font-bold text-khubrat-blue dark:text-khubrat-goldLight">Unified Evaluation Questions Template</h4>
          <p class="text-xs text-slate-400">
            <template v-if="workingTemplate">
              Establish standardized assessment questions for {{ workingTemplate.name }}.
            </template>
            <template v-else>
              Start by adding a template, then define criteria or import from archive.
            </template>
          </p>
        </div>
        ...
        <div class="flex items-center gap-2.5 flex-wrap">
          ...
          <BaseButton variant="ghost" @click="showCreateModal = true">
            <i class="fa-solid fa-folder-plus"></i> Add Template
          </BaseButton>

          <BaseButton variant="ghost" @click="openArchiveModal">
            <i class="fa-solid fa-clock-rotate-left"></i> Import from Archive
          </BaseButton>
    ...
    >
      ...
      <p class="font-bold text-sm">No evaluation template selected</p>
      <p class="text-xs text-slate-400 mt-1">Use Add Template or Import from Archive to get started.</p>
    </div>
    ...
    >
      <div v-if="!workingTemplate.questions?.length" class="text-center py-12 bg-slate-100/50 dark:bg-slate-900/30 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700">
        ...
        <h4 class="font-bold text-slate-500">No criteria defined yet</h4>
        <p class="text-xs text-slate-400">Click "Add Criterion" to formulate your evaluation questions.</p>
      </div>
        ...
        >
          <div class="flex items-start gap-3 w-full md:flex-1">
            ...
            <textarea
              ...
              placeholder="Enter evaluation criteria or question here…"
    ...
    >
      <p class="text-sm text-slate-500 dark:text-slate-300">Are you sure you want to delete this criteria question? This action cannot be undone.</p>
    </ConfirmModal>

    <ConfirmModal
      v-if="showCreateModal"
      title="Create New Template"
      confirm-label="Create Template"
      :loading="evaluationsStore.ActionLoading"
      @cancel="showCreateModal = false"
      @confirm="handleCreateTemplate"
    >
      <BaseInput v-model="newTemplateName" label="Template Name" placeholder="e.g. Standard Developer Evaluation" required />
    </ConfirmModal>

    <ConfirmModal
      v-if="showArchiveModal"
      title="Import Criteria from Archive"
      confirm-label="Import Selected Questions"
      :loading="evaluationsStore.ActionLoading || archiveLoading"
      @cancel="closeArchiveModal"
      @confirm="importQuestionsOnly"
    >
      <p class="text-xs text-slate-400 mb-3">Choose a historical template to append its questions to your current working template.</p>
      ...
      >
        ...
        <p class="font-bold">No archived templates found</p>
      ...
      <div v-if="archiveTemplateDetails?.questions?.length" class="mt-4 p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
        <h5 class="text-xs font-bold text-slate-500 mb-2">Questions to be imported:</h5>
        <ul class="space-y-2 text-xs text-slate-600 dark:text-slate-300">
          <li v-for="(q, idx) in archiveTemplateDetails.questions" :key="idx" class="flex gap-2">
            ...
            <span class="truncate">{{ q.question }} <span class="text-slate-400 opacity-70">({{ q.response_type }})</span></span>
          </li>
        </ul>
      </div>
      ...
      <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 flex justify-end">
        <BaseButton variant="ghost" size="sm" :disabled="!selectedArchiveTemplateId" @click="confirmArchiveImport">
          Duplicate as entirely new template instead
        </BaseButton>
      </div>
    </ConfirmModal>
  </div>
