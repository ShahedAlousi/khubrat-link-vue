  ...
  <div class="space-y-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div class="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm flex items-center justify-between">
        <div>
          <div class="text-xs text-slate-400">Present now</div>
          <div class="text-2xl font-bold text-khubrat-blue dark:text-white">{{ loading ? '—' : (headcount.present_now ?? 0) }}</div>
        </div>
      ...
      <div class="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm flex items-center justify-between">
        <div>
          <div class="text-xs text-slate-400">Late today</div>
          <div class="text-2xl font-bold text-slate-700 dark:text-slate-100">{{ loading ? '—' : (headcount.late_today ?? 0) }}</div>
        </div>
        <div class="ml-4 flex items-center">
      ...
      <div class="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm flex items-center justify-between">
        <div>
          <div class="text-xs text-slate-400">On leave today</div>
          <div class="text-2xl font-bold text-slate-700 dark:text-slate-100">{{ loading ? '—' : (headcount.on_leave_today ?? 0) }}</div>
        </div>
        <div class="ml-4 flex items-center">
    ...
    <div class="text-xs text-slate-400 mt-2">Last update: <span class="font-medium text-slate-600 dark:text-slate-300">{{ headcount.timestamp ?? '—' }}</span></div>
  </div>
</template>