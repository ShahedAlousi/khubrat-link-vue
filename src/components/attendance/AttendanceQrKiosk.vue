...
const clockTime = computed(() => {
  ...
  return { label: `${String(h).padStart(2, '0')}:${mm}:${ss}`, period }
})
const dateLabel = computed(() =>
  now.value.toLocaleDateString('en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })
)
      ...
      <div class="flex items-center justify-between px-8 py-5 border-b border-white/10">
        ...
        <div class="hidden md:flex items-center gap-4">
          <div class="text-center">
            <div class="flex items-baseline gap-2">
              <span class="text-[10px] font-black text-khubrat-goldLight">{{ clockTime.period }}</span>
          ...
          <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-400/30">
            ...
            <span class="text-[10px] font-bold text-emerald-300">Live • Attendance server connected</span>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="text-right hidden sm:block">
            <h1 class="text-sm font-extrabold text-khubrat-goldLight leading-tight">Khubrat HR Platform</h1>
            <p class="text-[9px] text-white/50">Live QR-based mobile attendance check-in</p>
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center max-w-6xl w-full">
          ...
          <div class="bg-slate-900/60 border border-khubrat-goldLight/20 rounded-3xl p-8 gold-glow mx-auto w-full max-w-md">
            <div class="flex items-center justify-between mb-6">
              <span class="px-2.5 py-1 rounded-lg bg-white/5 text-[10px] font-bold text-white/60">
                ID: {{ store.qrToken ? store.qrToken.slice(0, 12) : '—' }}
              </span>
              <span class="flex items-center gap-1.5 text-[10px] font-bold text-khubrat-goldLight">
                <i class="fa-solid fa-shield-halved"></i> Secure & Verified Code
              </span>
            </div>
            ...
            <div class="mt-6 space-y-2">
              <div class="flex items-center justify-between text-[10px] font-bold">
                <span class="text-white/50">Code expires in:</span>
                <span class="text-khubrat-goldLight">{{ store.qrExpiresIn }} seconds</span>
              </div>
          ...
          <div class="text-left space-y-5">
            ...
            <h2 class="text-3xl lg:text-4xl font-black leading-tight">
              Scan the QR with the<br />Employee App
            </h2>
            <p class="text-sm text-white/60 leading-relaxed max-w-md">
              Open the Khubrat app on your phone, then point your camera at the screen to check in or check out instantly and securely.
            </p>
            <ul class="space-y-3 text-xs font-semibold text-white/80">
              <li class="flex items-center gap-2">
                <i class="fa-solid fa-circle-check text-emerald-400"></i>
                Dynamic encrypted code regenerates automatically every 60 seconds for system protection.
              </li>
              <li class="flex items-center gap-2">
                <i class="fa-solid fa-circle-check text-emerald-400"></i>
                Verifies geographic location and live fingerprint instantly.
              </li>
            </ul>
          </div>
        </div>
