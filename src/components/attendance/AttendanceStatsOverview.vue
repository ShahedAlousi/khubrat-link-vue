<!--
  ...
  const kpiCards = computed(() => {
    ...
    return [
      ...
      { key: 'notArrived', label: 'Not Arrived', value: s.notArrived, percent: percentOf(s.notArrived), icon: 'fa-user-clock', color: 'sky' },
      { key: 'onLeave', label: 'On Leave', value: s.onLeave, percent: percentOf(s.onLeave), icon: 'fa-plane-departure', color: 'fuchsia' },
  ...
  const donutSegments = computed(() => {
    ...
    const defs = [
      { key: 'present', label: 'Present', count: d.present, percent: d.presentPercent, color: '#10b981', dot: 'bg-emerald-500' },
      { key: 'late', label: 'Late', count: d.late, percent: d.latePercent, color: '#f59e0b', dot: 'bg-amber-500' },
      { key: 'earlyLeave', label: 'Early Leave', count: d.earlyLeave, percent: d.earlyLeavePercent, color: '#6366f1', dot: 'bg-indigo-500' },
      { key: 'absent', label: 'Absent', count: d.absent, percent: d.absentPercent, color: '#f43f5e', dot: 'bg-rose-500' },
      { key: 'notArrived', label: 'Not Arrived', count: d.notArrived, percent: d.notArrivedPercent, color: '#0ea5e9', dot: 'bg-sky-500' },
      { key: 'onLeave', label: 'On Leave', count: d.onLeave, percent: d.onLeavePercent, color: '#d946ef', dot: 'bg-fuchsia-500' },
    ]
    let offset = 0
    return defs.map((seg) => {
    ...
    <section class="space-y-6">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[9px] text-slate-400 mt-0.5">{{ card.percent }}% of {{ store.stats.totalEmployees }} employees</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-6">
        <div class="space-y-2 flex-1 text-left">
          <h4 class="text-xs font-black text-khubrat-blue dark:text-khubrat-goldLight uppercase tracking-wider">Today's Status Breakdown</h4>
          <p class="text-[10px] text-slate-400 leading-relaxed">
            Share of every active employee by attendance status for the selected date/filters.
          </p>
        ...
        <div class="relative w-28 h-28 flex items-center justify-center shrink-0">
          ...
          <div class="absolute text-center">
            ...
            <span class="text-[7px] text-slate-400 font-semibold block uppercase">Present</span>
          </div>
        </div>
      </div>
