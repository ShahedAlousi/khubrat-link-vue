...
// ------------------------------------------------------------------------
// شارات الحالة وطريقة التوثيق (نفس منطق التصميم الأصلي، لكن مبنية على قيم
// status الفعلية القادمة من الباك اند: present/late/absent/early_leave/off_day)
// ------------------------------------------------------------------------
...
/** تنسيق وقت ISO كنص "hh:mm AM/PM" لعرضه بالجدول (أو "--:--" إن لم يوجد) */
function formatTime(iso) {
  if (!iso) return '--:--'
  const { hours, minutes, period } = parseTimeForClock(iso)
...
/** فتح مودال التعديل وتعبئته ببيانات السجل المُختار من الجدول */
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
      ...
      <div class="p-6 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="text-left">
          ...
          <p class="text-[10px] text-slate-400">Clock-in / clock-out activity for the selected date, with manual exception override.</p>
        </div>
        ...
        <table class="w-full text-left">
          <thead class="bg-slate-100/50 dark:bg-slate-900/40 text-slate-500 dark:text-slate-300 text-xs uppercase tracking-wider">
            <tr>
              ...
              <th class="p-4">Verification Method</th>
            ...
            >
              <td class="p-4 pl-6 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-[9px] text-slate-400">{{ rec.employeeTitle }}</p>
                </div>
              </td>
              ...
              <td class="p-4">
                <span v-if="rec.checkInMethod === 'qr'" class="text-slate-600 dark:text-slate-300 font-semibold flex items-center gap-1.5">
                  <i class="fa-solid fa-fingerprint text-emerald-500"></i> QR Stamp
                </span>
                <span v-else-if="rec.checkInMethod === 'manual'" class="text-khubrat-goldDark dark:text-khubrat-goldLight font-semibold flex items-center gap-1.5">
                  <i class="fa-solid fa-user-shield"></i> Manual Override
                </span>
              ...
              <td class="p-4 text-center pr-6">
                <button
                  ...
                  title="Record exceptional attendance override"
      ...
      >
        <span class="text-slate-400">
          Page {{ store.meta.current_page }} of {{ store.meta.last_page }} ({{ store.meta.total }} records)
        </span>
        ...
        <div class="p-6 space-y-4">
          <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-150">
            ...
            <div class="text-left">
              ...
              <p class="text-[10px] text-slate-400">{{ overrideForm.employeeTitle }}</p>
            </div>
          </div>
